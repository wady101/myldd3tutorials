 ...
 * semaphore held.
 */
int scull_trim(struct scull_dev *dev)                          /*Function is incharge of freeing space == Pg76 */
{
...
int scull_release(struct inode *inode, struct file *filp)     /* Used for shutting down stuff ; scull has no hw == Pg 73 */
{
	return 0;
}
/*
...
ssize_t scull_read(struct file *filp, char __user *buf, size_t count,
                loff_t *f_pos)                                                     /* Handles reading of files == Pg 81 */
{
	...
	if (*f_pos >= dev->size) 
		goto out;
	if (*f_pos + count > dev->size) //If difference is less than count
...
ssize_t scull_write(struct file *filp, const char __user *buf, size_t count,
                loff_t *f_pos)                                                   /*  Handles the writing of files == Pg 82 */
{
	struct scull_dev *dev = filp->private_data;
